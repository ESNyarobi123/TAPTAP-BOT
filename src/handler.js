...
async function handleEntry(sock, from, session, text) {
    ...
    try {
        ...
        if (result.type === 'waiter') {
            ...
            await sendText(sock, from, '❌ Invalid QR Code or Tag.');
...
async function handleStartState(sock, from, session, text) {
    ...
    if (greetings.includes(text.toLowerCase())) {
        await sendText(sock, from,
            '👋 Welcome to TipTap!\n' +
            'Which restaurant have you visited today? 🍽️\n\n' +
...
async function showHomeScreen(sock, from, session) {
    ...
    const rows = [
        ...
        { id: 'rate_service', title: '⭐ Rate Service', description: 'Give feedback' },
        { id: 'live_bill', title: '💳 Pay Bill', description: 'Pay your bill' },
        { id: 'give_tips', title: '💵 Tip', description: 'Tip the waiter' }
    ];
...
async function initiateQuickPayment(sock, from, session) {
    ...
    try {
        const result = await api.initiateQuickPayment({
            restaurant_id: session.restaurant_id,
        ...
        if (result.success) {
            ...
            await sendText(sock, from, '❌ Tatizo la kuanzisha malipo. Jaribu tena.');
            await showHomeScreen(sock, from, session);
        }
    } catch (e) {
        console.error('Quick Payment Error:', e);
        await sendText(sock, from, '❌ Tatizo la kiufundi.');
        await showHomeScreen(sock, from, session);
    }
}
